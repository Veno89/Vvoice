...
import type { ConnectionState } from '../ws/message-handler.js';
...
export async function registerApiRoutes(
  ...
  // Kick User
  app.post('/api/admin/kick', { preHandler: requireAdmin }, async (request: any, reply) => {
  ...
  // Ban User
  app.post('/api/admin/ban', { preHandler: requireAdmin }, async (request: any, reply) => {
    const { userId, reason } = z.object({ userId: z.string(), reason: z.string().optional() }).parse(request.body);
  ...
  app.post('/api/auth/register', async (request, reply) => {
    ...
    const token = signToken(cfg.jwtSecret, user.id, user.username, user.role as 'admin' | 'member');
