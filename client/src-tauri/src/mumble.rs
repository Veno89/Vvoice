...
fn parse_expected_cert_sha256() -> Result<Option<String>> {
    match std::env::var("VVOICE_TLS_CERT_SHA256") {
        Ok(value) => {
            let normalized = value.trim().to_ascii_lowercase().replace(':', "");
            if normalized.len() != 64 || !normalized.chars().all(|c| c.is_ascii_hexdigit()) {
                anyhow::bail!(
                    "VVOICE_TLS_CERT_SHA256 must be a 64-character SHA-256 hex digest (colons optional)"
                );
            }
            Ok(Some(normalized))
        }
        ...
        Err(e) => Err(anyhow::anyhow!(
            "Failed to read VVOICE_TLS_CERT_SHA256: {e}"
        )),
    }
}

fn try_enqueue_packet(
    tx: &tokio::sync::mpsc::Sender<MumblePacket>,
    packet: MumblePacket,
    context: &str,
) {
    if let Err(e) = tx.try_send(packet) {
        tracing::warn!("Dropping outbound packet in {context}: {e}");
    }
}
fn verify_peer_certificate_fingerprint(
    ...
    let certs = connection
        .peer_certificates()
        .ok_or_else(|| anyhow::anyhow!("TLS peer certificate chain is missing"))?;
    let leaf = certs
        .first()
        .ok_or_else(|| anyhow::anyhow!("TLS peer did not present a leaf certificate"))?;

    let actual = format!("{:x}", Sha256::digest(leaf.as_ref()));
    if actual != expected_sha256 {
        anyhow::bail!(
            "TLS certificate fingerprint mismatch: expected {expected_sha256}, got {actual}"
        );
    }

    tracing::info!("TLS certificate fingerprint matched configured pin");
    Ok(())
}
...
impl VoiceClient {
    ...
    pub async fn connect(
        ...
        let expected_cert_sha256 = parse_expected_cert_sha256()?;
        ...
        verify_peer_certificate_fingerprint(&tls_stream, expected_cert_sha256.as_deref())?;
                    ...
                    if i16_samples.len() >= 480 {
                        ...
                        match encoder.encode(&i16_samples, &mut output) {
                            Ok(len) => {
                                ...
                                try_enqueue_packet(
                                    &tx_audio,
                                    MumblePacket::UDPTunnel(tunnel_msg),
                                    "audio_capture",
                                );
                            }
                            Err(_e) => {}
    ...
    pub fn send_message(&self, message: String) {
        ...
        msg.message = message;
        try_enqueue_packet(&self.tx, MumblePacket::TextMessage(msg), "send_message");
    }
    ...
    pub fn join_channel(&self, channel_id: u32) {
        ...
        try_enqueue_packet(&self.tx, MumblePacket::UserState(msg), "user_state_update");
    }
